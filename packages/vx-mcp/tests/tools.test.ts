...
const RPC_SRC = readFileSync(path.join(SRC_DIR, 'tools.ts'), 'utf8')
const MCP_SRC = readFileSync(path.join(SRC_DIR, 'index.ts'), 'utf8')
...
describe('the tool listing and the dispatcher describe the same set', () => {
  ...
  function dispatchedTools(): string[] {
    const fn = /export async function handleToolCall\([\s\S]*?\n\}/.exec(RPC_SRC)
    if (fn === null) {
      throw new Error(
        'tools drift guard: could not find `export async function handleToolCall` — the ' +
...
describe('getCacheStats — argument validation at the model boundary', () => {
  ...
  it('an entirely absent argument object is the same as no scope', async () => {
    ...
    const a = (await call(MAIN.root, 'getCacheStats', undefined)) as Stats
    const b = (await call(MAIN.root, 'getCacheStats', null)) as Stats
...
describe('explainCacheKey', () => {
  ...
  it('says plainly that the input components are not in this answer', async () => {
    ...
    expect(got.note).toMatch(/per-component input breakdown is `vx why/)
  })
...
describe('the workspace context decides which database is read', () => {
  it('two contexts read two different databases', async () => {
    // A handler reads exactly the cache dir it is handed — the workspace's
    // declared `cacheDir` is resolved by core's command context, pinned
