...
const DOCS_DIR = path.resolve(HERE, '../../vx/docs')
const OUT_DIR = path.resolve(HERE, '../src/content/docs')

/** A YAML comment in every generated page's frontmatter — the only way a page says it is generated. */
const GENERATED_MARK = '# generated by packages/vx-docs/scripts/import-docs.ts from packages/vx/docs/ — edit the source, not this file'
...
for (const f of new Glob('*.md').scanSync({ cwd: OUT_DIR })) {
  const head = (await Bun.file(path.join(OUT_DIR, f)).text()).slice(0, 400)
  if (head.includes(GENERATED_MARK)) await rm(path.join(OUT_DIR, f), { force: true })
}
for (const entry of ['modules', 'design']) {
