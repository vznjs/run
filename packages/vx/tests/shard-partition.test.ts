...
describe('weighted test shards', () => {
  ...
  it('deals every non-unsafe test file exactly once across 8 shards', () => {
    const bins = partition(files, weights, 8)
  ...
  it('is deterministic', () => {
    expect(partition(files, weights, 8)).toEqual(partition(files, weights, 8))
  })
  ...
  it('balances the recorded load: heaviest shard within 1.25× of the lightest', () => {
    ...
    const loads = partition(files, weights, 8).map((bin) =>
